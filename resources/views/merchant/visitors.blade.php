					...
					<div class="table-wrap mb-20">
                                    <div class="table-wrap">
                                        <table id="datable_3" class="table table-hover w-100 display">
                                            ...
                                            <tbody>
                                                @foreach (App\Visitor::where('merchant_id', Auth::user()->id)->orderBy('id', 'asc')->get() as $user)
                                                <tr>
                                                    <td>{{ $loop->index + 1 }}</td>                            
                                                    <td><a href="{{ route('merchant.visitorShow', $user->id ) }}">{{$user->username}}</a></td>
                                                    <td>{{ $user->name }}</td>
                                                    <td>{{ $user->email }}</td>
                                                    <td>{{ $user->phone }}</td>
                                                    <td>                                                        
                                                    ...
                                                    <td>
                                                        <form action="{{ route('merchant.visitorStatus', $user->id) }}" method="POST">
                                                            @csrf
